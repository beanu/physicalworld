...
public class DistanceJointHelper implements Shape {
	...
	public DistanceJointHelper(Box2dAdapter adapter) {
	...
	public void render(float delta) {
		if (data1 != null) {
			...
			render.line(firstPoint.x, firstPoint.y, secondPoint.x,
					secondPoint.y);
			render.end();
				...
				for (BodyData d : adapter.data.bodyDatas) {
					if (d.isFocus(Engine.screenToWorld(x, y))) {
						if (data1 != null && data1 != d) {
							data2 = d;
							secondPoint.set(Engine.screenToWorld(x, y));
							data = new DistanceJointData();
							data.bodyA = data1;
							data.bodyB = data2;
							data.localAnchorA.set(firstPoint).sub(data1.center);
							data.localAnchorB.set(secondPoint)
									.sub(data2.center);
